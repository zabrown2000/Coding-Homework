...
/// <returns>Node*</returns>
template<class T>
typename Tree<T>::Node* SearchTree<T>::findParent(typename Tree<T>::Node* current, typename Tree<T>::Node* child) {   
	if (current == nullptr) return nullptr; //empty tree								
	...
	if (current->value > child->value) { //current root is greater than searching value so look to left
		return findParent(current->left, child);
	}
	if (current->value <= child->value) { //current root is less than or equal to searching value so look to right
